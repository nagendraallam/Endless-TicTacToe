...
module.exports = function (io) {
  setInterval(() => {
    ...
    Object.keys(rooms).forEach((roomId) => {
      ...
      if (
        (room.players.length === 1 && room.players[0].role) ||
        (room.players[1].username === "Bot" &&
          room.currentTurn !== room.players[0].role)
      ) {
    ...
    socket.on("makeMove", ({ index, roomId }) => {
      ...
      if (player && player.role === currentTurn && !board[index]) {
        ...
        if (checkWin(board, currentTurn)) {
          ...
          boardState[roomId] = Array(9).fill(null); // Reset the board for a new game
          rooms[roomId].players = []; // Reset players array if necessary
        } else {
